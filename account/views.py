...
from account.models import Address, DeliveryCost, Profile, User
...
from core.permissions import (
    address_object_level_permissions,
...
class SendOtpCode(APIView):
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        if request_type not in allowed_reqeust_type:
            return Response(
                {"error": "Please send correct request type"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        phone_number = str(serializer.validated_data["phone_number"]).replace(
            "+98", "0"
        )
        print(phone_number)
        ...
        if cached_data:
            ...
            if reminded_time:
                return Response(
                    {
                        "error": f'OTP request limit reached. Try again after {reminded_time} {"second" if reminded_time == 1 else "seconds"}.'
                    },
        ...
        return Response(
            {
                "token": phone_hash,
                "message": "Otp send request recieved successfully!",
            },
...
class VerifyOtp(APIView):
    ...
    def post(self, request):
        ...
        if not cached_data:
            return Response(
                {"message": "Your code is expired please try to get new OTP code."},
        ...
        if cached_otp != str(otp):
            return Response(
                {"error": "Sended otp is not correct!"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        serializer = VerifyOtpSerializer(
            data=request.data, context={"request_type": cached_request_type}
        )
        serializer.is_valid(raise_exception=True)
        if cached_request_type == "login_register":
            ...
            if not user:
                try:
                    ...
                    return Response(
                        {"error": "Error when create new user!"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
        ...
        if cached_request_type == "set_change_pass":
            ...
            if not user:
                return Response(
                    {"error": f"There is no user with this {phone_number}"},
        ...
        if profile:
            if profile.user == self.request.user:
                ...
                raise PermissionDenied(
                    "You do not have permission to access addresses for this profile."
                )
        else:
